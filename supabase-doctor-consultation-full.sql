-- ================================================================
-- ZED Doctor Consultation Full Schema (separate file)
-- Includes onboarding, verification, booking, payment, chat, rating, ranking
...
CREATE INDEX IF NOT EXISTS bookings_time_idx ON public.bookings(scheduled_time ASC);

ALTER TABLE public.bookings ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Bookings participant access" ON public.bookings;
CREATE POLICY "Bookings participant access"
...
CREATE POLICY "Messages participant access"
  ...
  USING (
    ...
    OR auth.uid() = (SELECT b.patient_id FROM public.bookings b WHERE b.id = booking_id LIMIT 1)
    OR auth.uid() = (
      SELECT d.user_id
...
CREATE POLICY "Payments participant read"
  ...
  USING (
    auth.uid() = (SELECT b.patient_id FROM public.bookings b WHERE b.id = booking_id LIMIT 1)
    OR auth.uid() = (
      SELECT d.user_id
      FROM public.bookings b
      JOIN public.doctors d ON d.id = b.doctor_id
