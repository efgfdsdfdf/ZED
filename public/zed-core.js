...
const SUPABASE_ANON = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNlbnBsYnd' + 'wanljeG90Y3R2am16Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI5MDE0NDgsImV4cCI6MjA4ODQ3NzQ0OH0.6lDMcolkeHre8VE7R823pMcx3uA6Rvw2C9XTiWtUvD8';
...
function zedCandidateApiUrls(path) {
  ...
  const hasOrigin = protocol !== 'file:' && origin && origin !== 'null';

  if (hasOrigin && port === '3000') {
  ...
  if (protocol === 'file:' || isLocalHost) {
    ...
    // If running frontend on another local port (e.g. 5502/5173), try backend first.
    if (hasOrigin && port !== '3000') {
      urls.push(`${origin}${path}`);
    }
  } else if (hasOrigin) {
...
const ZedAI = {
  ...
  async _post(body) {
    ...
    let session = null;
    try {
      ({ session } = await ZedAuth.getSession());
    } catch (_) {
      session = null;
    }
    ...
    for (const url of this._apiUrls) {
      try {
        ...
        if (!res.ok) {
          ...
          console.warn(`API call failed at ${url} (${res.status}). Trying next endpoint...`);
          continue;
        }

        console.log(`AI API connected: ${url}`);
        this._apiUrl = url;
        return data;
      } catch (error) {
        lastError = error;
        console.warn(`API call network failure at ${url}. Trying next endpoint...`);
      }
    }
